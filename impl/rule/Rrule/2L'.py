    ...
    def create_constraints(self, board: 'AbstractBoard') -> bool:
        ...
        for pos, _ in board(key=NAME_2L):
            ...
            for key in board.get_interactive_keys():
                ...
                # model.Add(var == 0).OnlyEnforceIf(key_var)
...
class Value2L(AbstractClueValue):
    ...
    def create_constraints(self, board: 'AbstractBoard'):
        ...
        model.Add(sum(nei_vars) == self.value).OnlyEnforceIf(var)

        tmp_a = model.NewBoolVar("tmp_a")
        tmp_b = model.NewBoolVar("tmp_b")

        model.Add(sum(nei_vars) == self.value + 1).OnlyEnforceIf([tmp_a, var.Not()])
        model.Add(sum(nei_vars) == self.value - 1).OnlyEnforceIf([tmp_b, var.Not()])
