    ...
    def create_constraints(self, board: 'AbstractBoard'):
        ...
        for index in range(len(last_row)):
            ...
            for index_t in range(len(col_b) - 1):
                ...
                model.AddBoolOr([var_t_a, var_t_b]).OnlyEnforceIf(tmp_t)
                model.AddBoolAnd([var_t_a.Not(), var_t_b.Not()]).OnlyEnforceIf(tmp_t.Not())
            ...
            for index_a in range(len(col_b) - 1):
                for index_b in range(index_a + 1, len(col_b)):
                    if -1 < index - 1 < len(last_row):
                        ...
                        ).OnlyEnforceIf(col_var[index_a+1:index_b-1] + [col_var[index_a].Not(), col_var[index_b-1].Not()])
                    if -1 < index + 1 < len(last_row):
            ...
            for index_t in range(1, len(col_b) - 1):
                if -1 < index - 1 < len(last_row):
                    ...
                    ).OnlyEnforceIf(col_var[:index_t]+[col_var[index_t].Not()])
                if -1 < index + 1 < len(last_row):
                    col_a = board.batch(board.get_col_pos(last_row[index + 1]), mode="variable")
                    model.Add(
                        sum(col_a[:index_t+1]) ==
            ...
            for index_t in range(1, len(col_b) - 1):
                if -1 < index - 1 < len(last_row):
                    ...
                    ).OnlyEnforceIf(col_var[index_t:] + [col_var[index_t-1].Not()])
                if -1 < index + 1 < len(last_row):
                    col_a = board.batch(board.get_col_pos(last_row[index + 1]), mode="variable")
                    model.Add(
                        sum(col_a[index_t:]) ==
        ...
        for pos in board.get_col_pos(boundary_pos):
            ...
            for index in range(len(row) - 1):
                ...
                model.Add(sum(vars_a) == sum(vars_b)).OnlyEnforceIf([var_a.Not(), var_b.Not()])
